# purpose: Compare OLS, GLS AR(1), and GLS AR(2) under common mean structures for the shift and trend models.
# inputs: model_df from prepare_model_data(), containing year, season, metric, period, and temperature columns.
# outputs: Detail and summary tibbles returned by run_model_comparison() and related helper functions.
# how it is called: Sourced by reports/ST422_Professional_Technical_report.Rmd for model comparison and dependence-order checks.
...
# Safely extract a chosen sample autocorrelation lag from a residual vector.
...
# Safely compute a Ljung-Box p-value when enough residuals are available.
...
# Compute a simple rolling one-step-ahead RMSE for a supplied fitting function.
compute_rolling_rmse <- function(data_df, fit_fun, min_train = 30) {
  ...
  for (i in seq(min_train, n - 1)) {
    # Split the series into an expanding training sample and a one-step test point.
    train_df <- data_df[1:i, , drop = FALSE]
    test_df  <- data_df[i + 1, , drop = FALSE]

    # Fit the model safely so failed windows do not stop the whole comparison.
    ...
    # Generate a one-step forecast and skip windows that do not return a finite value.
    ...
    # Store the squared one-step prediction error for RMSE calculation.
  ...
  # Return NA if no valid prediction windows were available.
  if (length(sq_err) == 0) {
    return(NA_real_)
  }
...
# Extract common fit statistics from an OLS model object.
...
# Extract common fit statistics from a GLS model object.
...
# Compare OLS, GLS AR(1), and GLS AR(2) for one season-metric series.
compare_one_series <- function(df_group, model_type = c("shift", "trend")) {
  model_type <- match.arg(model_type)

  if (model_type == "shift") {
    # Build the shift-model dataset using only the historical and recent windows.
    g <- df_group %>%
    ...
    # Build the trend-model dataset using centred year over the full available record.
    g <- df_group %>%
      dplyr::arrange(year) %>%
  ...
  # Fit the three candidate model families under a common mean structure.
  fit_ols <- tryCatch(
  ...
  # Convert each fitted model into a common summary row, or return NA rows if fitting failed.
  ...
  if (model_type == "trend") {
    # Add rolling forecast error summaries for the long-run trend comparison only.
    ...
    # Scale RMSE by the series standard deviation to form a comparable NRMSE.
...
# Run the chosen model family comparison across all season-metric combinations.
run_family_model_comparison <- function(model_df,
  ...
  if (!is.null(year_min)) {
    ...
    # Restrict the comparison to the requested lower year bound when supplied.
    model_df <- model_df %>% dplyr::filter(year >= year_min)
  }

  if (!is.null(year_max)) {

    # Restrict the comparison to the requested upper year bound when supplied.
    model_df <- model_df %>% dplyr::filter(year <= year_max)
  }

  # Build the series-level comparison table first.
  detail_tbl <- model_df %>%
    dplyr::group_by(metric, season) %>%
    dplyr::group_modify(~ compare_one_series(.x, model_type = model_type)) %>%
    dplyr::ungroup()

  # Collapse the series-level comparison into a model-family summary table.
...
# Format the summary comparison table for report presentation.
format_model_comparison_summary <- function(summary_tbl, digits = 3) {
  # Format finite numeric values while preserving NA entries as character strings.
  fmt_num <- function(x) {
    out <- rep("NA", length(x))
    ok <- is.finite(x)
  ...
  # Recode model labels and convert summary statistics to display strings.
  out <- summary_tbl %>%
    dplyr::mutate(
  ...
  # Select and rename the final presentation columns in the desired order.
  out <- out %>%
    dplyr::select(
      specification,
      mean_aic,
...
# Run both the shift-model and trend-model comparison pipelines and return both outputs.
